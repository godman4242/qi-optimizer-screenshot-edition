...
- **Rarity colour has one source.** The `--rarity-C/U/R/E/L` palette and the `.rarity-X` classes are published as a `--rarity` custom property, so any new surface opts in with `color: var(--rarity)` rather than hard-coding a hex. Rarity for a NAMED herb always comes from `data.js`, never from the tile colour the OCR sampled — that was only ever a hint for reading the name.
- **Colour is never the only signal.** Anywhere rarity is shown by colour it is also shown by a heading or a letter chip, and `tests/ui-smoke.mjs` asserts the rendered colours differ so a class with no matching rule cannot pass silently.
